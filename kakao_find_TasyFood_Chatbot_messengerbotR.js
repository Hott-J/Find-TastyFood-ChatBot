...
function kakao_login(){ //카카오링크 사용을 위해 로그인 세션이 만료되지 않게하기 위함
  try{
    Kakao.init('myJS_Key'); // 중요포인트 : 반드시 봇계정 카카오아이디와 패스워드로 카카오디벨로퍼에 로그인하여 자바스크립트 키값을 받아올것!
    Kakao.login('myKakao_Id','myKakao_password');//중요포인트 : 반드시 봇계정 카카오아이디와 패스워드를 적어줄것!!
...
function store(){
  ...
  for(i=0;i<5;i++){
    try{
      ...
      // 가게이름에 특수문자 제거,//위치 역시 지역이 3개이상시 검색이 안되길래, 위치가 방배/반포/잠원 이런식으로 '/'로 구분되어있어서, 지역하나와 특수문자가 제거된 가게이름으로 검색하여 오류 제거
...
function response(room, msg, sender, isGroupChat, replier, ImageDB, packageName, threadId){
  if (msg.startsWith("!맛집")) {
    ...
    send_template(room, myTemplate_Id,set); /*카카오 디벨로퍼에 만든 본인 템플릿 형식으로 보냄
